...
class ContagemPalavrasTest < Minitest::Test
  ...
  def test_com_apostofros
    skip
    frase = Frase.new("First: don't laugh. Then: don't cry.")
    contagem = {"first" => 1, "don't" => 2, "laugh" => 1, "then" => 1,
                "cry" => 1 }
    assert_equal contagem, frase.conte_palavras
  end
  ...
  def test_com_aspas
    skip
    frase = Frase.new("Você sabe a diferença entre 'grande' e grande.")
    contagem = { "Você" => 1, "sabe"=> 1, "a"=>1, "diferença" => 1,
                 "entre" => 1, "grande" => 2, "e" => 1 }
    assert_equal contagem, frase.conte_palavras
  end
end
